...
const HEADER_PROTECT = "page-break-inside:avoid;break-inside:avoid;page-break-after:avoid;break-after:avoid";
const PARA_PROTECT = "page-break-inside:avoid;break-inside:avoid";
...
function paragraphs(content: string): string {
  const c = content ?? "";
  if (isHtmlContent(c)) {
    return `<div class="cl-html-body">${c}</div>`;
  }
  return c.split("\n").filter((p: string) => p.trim())
    .map((p: string) => `<p style="margin-bottom:14px;font-size:12px;line-height:1.8;color:#374151;${PARA_PROTECT}">${p}</p>`)
    .join("");
}
...
function buildHtml(content: string, templateId: string, accentColor: string, fontFamily: string, jobTitle: string, company: string, letterTitle: string, personal: any): string {
  ...
  if (templateId === "modern") {
    body = `
    <table style="width:100%;table-layout:fixed;min-height:calc(297mm - 2mm);border-collapse:collapse">
      <tr>
        <td style="width:35%;background:${accentColor};vertical-align:top;padding:28px 16px">
          <div style="${HEADER_PROTECT}">
    ...
    body = `
    <div style="${HEADER_PROTECT};background:${accentColor};padding:24px 32px;color:#fff;display:flex;align-items:center;justify-content:space-between">
      <div style="flex:1">
    ...
    body = `
    <div style="border-left:5px solid ${accentColor};padding:28px 32px 28px 40px;min-height:calc(297mm - 2mm)">
      <div style="${HEADER_PROTECT}">
    ...
    body = `
    <div style="padding:20px 28px;min-height:calc(297mm - 2mm)">
      <div style="${HEADER_PROTECT}">
    ...
    body = `
    <div style="padding:28px 32px;min-height:calc(297mm - 2mm)">
      <div style="${HEADER_PROTECT}">
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
  ...
  .cl-html-body p { margin: 0 0 14px; }
  .cl-html-body ul { margin: 0 0 14px 20px; padding: 0; list-style: disc; }
  .cl-html-body ol { margin: 0 0 14px 20px; padding: 0; list-style: decimal; }
  .cl-html-body li { margin-bottom: 4px; }
  .cl-html-body strong { font-weight: 700; }
  .cl-html-body em { font-style: italic; }
  .cl-html-body u { text-decoration: underline; }
  .cl-html-body p, .cl-html-body li { ${PARA_PROTECT}; }
</style>
</head>
<body>${body}</body>
</html>`;
