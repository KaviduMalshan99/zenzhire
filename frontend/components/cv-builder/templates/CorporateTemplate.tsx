...
// RIGHT column in this template (MIRRORED vs Portrait/Milestone where these live on the LEFT).
// Education/Skills/Languages are the "at-a-glance" sidebar here; Summary/Experience are main content on the left.
...
export const CORPORATE_SIDEBAR_TYPES = new Set(["education", "skills", "soft_skills", "certificates", "languages", "interests"]);
const SIDEBAR_TYPES = CORPORATE_SIDEBAR_TYPES;
...
function getContactIcon(type: string, fill: string): React.ReactNode {
  ...
  switch (type) {
    ...
    default:         return <svg style={s} width="11" height="11" viewBox="0 0 24 24" fill={fill}><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/></svg>;
  }
}

// Decorative scattered dot grid — pure SVG, no image. Positioned in top-right of header.
function DotPattern({ color }: { color: string }) {
  const dots = [
    [8,6],[22,4],[38,8],[54,5],[70,9],[82,5],
    [4,18],[16,22],[32,18],[48,20],[64,16],[78,22],
    [10,32],[24,36],[40,30],[56,34],[72,30],[86,36],
    [6,46],[18,50],[34,44],[50,48],[66,44],[80,50],
    [12,60],[26,64],[42,58],[58,62],[74,58],[88,64],
  ];
  return (
    <svg width="96" height="72" viewBox="0 0 96 72" aria-hidden="true" style={{ flexShrink: 0, opacity: 0.22 }}>
      {dots.map(([x, y], i) => <circle key={i} cx={x} cy={y} r={2.5} fill={color} />)}
    </svg>
  );
}
...
function CH({ title, section, accentColor, fontCSS, sectionSpacing }: CHProps) {
  ...
  return (
    ...
    >
      <span style={{ color: accentColor, fontSize: 14, lineHeight: 1, flexShrink: 0 }}>⊙</span>
...
export function CorporateTemplate({ sections, customization = DEFAULT_CUSTOMIZATION }: Props) {
  ...
  // SIDEBAR_TYPES go to the RIGHT column (opposite of Portrait/Milestone where they go left).
  // References is pulled out to render full-width below the 2-column area.
  ...
  const renderSidebarSection = (section: CVSection) => {
    ...
    switch (section.section_type) {
      ...
      // Skills / Soft Skills: plain bullet list, name only.
      // This template intentionally ignores the skillStyle customization option — the sidebar's
      // narrow width and clean corporate aesthetic call for simple bullets regardless of user preference.
      case "skills":
      case "soft_skills":
        if (!entries.length) return null;
        return (
      ...
      // Languages: plain bullet list, name only.
      // Same as Skills above — skillStyle is intentionally ignored for this sidebar column.
      case "languages": {
        if (!entries.length) return null;
  ...
  const renderMainSection = (section: CVSection) => {
    ...
    switch (section.section_type) {
      ...
      // Work Experience: job title bold, company + dates on next line, bullets below. No timeline markers.
      case "experience": {
        if (!entries.length) return null;
        const renderEntry = (entry: any, i: number) => (
          <div key={i} style={{ marginBottom: Math.round(12 * sp), fontFamily: fontCSS, ...eb }} className="cv-entry">
            ...
            <div style={{ fontSize: 11.5, color: MID, fontFamily: fontCSS, marginTop: 1 }}>
              ...
              {entry.employer && (entry.start_date || entry.end_date || entry.current) && " · "}
  ...
  return (
    <div className="corporate-outer" style={{ backgroundColor: "#fff", fontSize: 12, color: MID, lineHeight: 1.6, fontFamily: fontCSS, minHeight: "297mm" }}>
      {/* Header: name/title/contact left-aligned, dot pattern top-right */}
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", padding: `${headerPad}px 32px 18px` }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 32, fontWeight: 800, letterSpacing: "0.01em", lineHeight: 1.1, color: DARK, fontFamily: fontCSS }}>
            <EditableText value={personal.full_name} onCommit={(v) => setPersonal("full_name", v)} placeholder="Your Name" />
          </div>
          {personal.title && (
            <div style={{ fontSize: 14, color: LIGHT, marginTop: 5, fontFamily: fontCSS }}>
          ...
          {contactItems.length > 0 && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 16px", marginTop: 10 }}>
              {contactItems.map((item, i) => (
                <span key={i} style={{ display: "inline-flex", alignItems: "center", fontSize: 11, color: MID, fontFamily: fontCSS }}>
      ...
      {/* Body: LEFT = main (~65%), RIGHT = sidebar (~35%) with left border */}
      ...
      {/* References: full-width at bottom, outside the 2-column area */}
      {referencesSection && (
