  ...
  return (
    <div style={{ paddingBottom: Math.round(24 * sp), fontSize: 12, color: MID, lineHeight: 1.6, fontFamily: fontCSS }}>
      ...
      <div style={{ backgroundColor: "#f0f4f8", borderBottom: "1px solid #dde3ea", padding: `${Math.round(8 * sp)}px 16px`, display: "flex", flexWrap: "wrap", gap: "4px 20px", fontSize: 10, color: LIGHT, fontFamily: fontCSS }}>
        {personal.email && <span style={{ display: "inline-flex", alignItems: "center" }}>{getContactIcon("email", LIGHT)}<a href={`mailto:${personal.email}`} target="_blank" rel="noopener noreferrer" style={{ color: "inherit", textDecoration: "none" }}><EditableText value={personal.email} onCommit={(v) => setPersonal("email", v)} /></a></span>}
        {personal.phone && <span style={{ display: "inline-flex", alignItems: "center" }}>{getContactIcon("phone", LIGHT)}<a href={`tel:${personal.phone}`} target="_blank" rel="noopener noreferrer" style={{ color: "inherit", textDecoration: "none" }}><EditableText value={personal.phone} onCommit={(v) => setPersonal("phone", v)} /></a></span>}
        {personal.location && <span style={{ display: "inline-flex", alignItems: "center" }}>{getContactIcon("location", LIGHT)}<EditableText value={personal.location} onCommit={(v) => setPersonal("location", v)} /></span>}
        {links.map((l: any, i: number) => l.url && (
          <span key={i} style={{ display: "inline-flex", alignItems: "center" }}>
            {getContactIcon((l.platform ?? "").toLowerCase().includes("linkedin") ? "linkedin" : (l.platform ?? "").toLowerCase().includes("github") ? "github" : "website", LIGHT)}
            <a href={l.url.startsWith("http") ? l.url : `https://${l.url}`} target="_blank" rel="noopener noreferrer" style={{ color: "inherit", textDecoration: "none" }}><EditableText value={l.url} onCommit={(v) => setPersonal("links", links.map((x: any, xi: number) => (xi === i ? { ...x, url: v } : x)))} /></a>
          </span>
        ))}
      </div>
